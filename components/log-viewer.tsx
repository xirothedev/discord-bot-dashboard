...
export function LogViewer({ bots }: LogViewerProps) {
  ...
  useEffect(() => {
    const generateSampleLogs = () => {
      ...
      const levels: LogEntryType["level"][] = ["info", "warn", "error", "debug", "success"]
      const categories: LogEntryType["category"][] = ["system", "command", "error", "user", "api", "database"]
      ...
      for (let i = 0; i < 50; i++) {
        ...
        initialLogs.push({
          ...
          channelId: Math.random() > 0.5 ? `channel_${Math.floor(Math.random() * 100)}` : undefined,
          guildId: Math.random() > 0.5 ? `guild_${Math.floor(Math.random() * 50)}` : undefined,
        })
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const newLog: LogEntryType = {
        ...
        channelId: Math.random() > 0.7 ? `channel_${Math.floor(Math.random() * 100)}` : undefined,
      }
  ...
  return (
    <div className="space-y-4">
      <LogFilters filters={filters} onFiltersChange={setFilters} botOptions={bots} />

      <Card className="cyber-gradient border cyber-border">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-lg font-semibold text-purple-400">System Logs ({filteredLogs.length})</CardTitle>
            <div className="flex items-center gap-2">
              ...
              <Button
                ...
                className={`hover:bg-purple-500/10 hover:border-purple-500/50 ${
                  isStreaming ? "bg-green-500/10 border-green-500/30" : "bg-red-500/10 border-red-500/30"
                }`}
              >
