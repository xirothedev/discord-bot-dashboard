...
export default function Page() {
  ...
  const [systemMetrics, setSystemMetrics] = useState<MonitorApiResponse | null>(
    null
  );
  ...
  useEffect(() => {
    if (data) {
      ...
      const cpu =
        Array.isArray(data.loadAverage) &&
        data.loadAverage.length > 0 &&
        data.cpu.count
  ...
  const cpuUsage = systemMetrics
    ? Array.isArray(systemMetrics.loadAverage) &&
      systemMetrics.loadAverage.length > 0
      ? Number(
          (
            (systemMetrics.loadAverage[0] * 100) /
            systemMetrics.cpu.count
          ).toFixed(2)
        )
      : 0
    : 0;
